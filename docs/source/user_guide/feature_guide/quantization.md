...
```bash
# Install tag_MindStudio_8.3.T1.B080_001 version
git clone https://gitcode.com/Ascend/msit.git -b tag_MindStudio_8.3.T1.B080_001
...
Once you have a quantized model, you can use vLLM Ascend for inference by specifying the `--quantization ascend` parameter to enable quantization features.
...
llm = LLM(model="/path/to/your/quantized_model",
          ...
          # Specify `quantization="ascend"` to enable quantization
          quantization="ascend")
